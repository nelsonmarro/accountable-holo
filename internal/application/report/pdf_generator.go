...
func (g *PDFReportGenerator) SelectedTransactionsReport(ctx context.Context, transactions []domain.Transaction, outputPath string, currentUser *domain.User) error {
	...
	if err := g.buildFooter(m, currentUser); err != nil {
		return err
	}
...
func (g *PDFReportGenerator) ReconciliationStatementReport(ctx context.Context, reconciliation *domain.Reconciliation, outputPath string, currentUser *domain.User) error {
	cfg := config.NewBuilder().
		WithPageNumber().
		WithLeftMargin(10).
		WithTopMargin(15).
...
func (g *PDFReportGenerator) DailyReport(ctx context.Context, report *domain.DailyReport, outputPath string, currentUser *domain.User) error {
	cfg := config.NewBuilder().
		WithPageNumber().
		WithLeftMargin(10).
		WithTopMargin(15).
...
func (g *PDFReportGenerator) buildFooter(m core.Maroto, currentUser *domain.User) error {
	...
	return m.RegisterFooter(
		...
		// Generated By Row
		row.New(10).Add(
			text.NewCol(12, fmt.Sprintf("Reporte Generado Por: %s %s", currentUser.FirstName, currentUser.LastName), legendTextProps),
		),
	)
}
